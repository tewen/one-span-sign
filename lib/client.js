...
const requestProperties = (instance) => _.pick(instance, ['apiKey', 'sandbox', 'domain', 'sandboxDomain']);

class Client {
  constructor({ apiKey, sandbox, domain = DEFAULT_DOMAIN, sandboxDomain = DEFAULT_SANDBOX_DOMAIN }) {
    if (!_.isString(apiKey)) {
      ...
      throw new InitializationError(`You must pass an domain:String property to the client constructor. The default is '${DEFAULT_DOMAIN}'. Refer to ${API_URLS} for more information.`);
    } else if (sandbox && (!_.isString(sandboxDomain) || _.isEmpty(sandboxDomain))) {
      throw new InitializationError(`You must pass an sandboxDomain:String property to the client constructor. The default is '${DEFAULT_SANDBOX_DOMAIN}'. Refer to ${API_URLS} for more information.`);
    }
    this.apiKey = apiKey;
